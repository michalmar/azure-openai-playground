...
### Prompt files
# prompt file is a TXT file that contains the prompts for the user to answer
def get_all_prompt_files():
    # Get all files in the container from Azure Blob Storage
    account_name = os.environ['BLOB_ACCOUNT_NAME']
    account_key = os.environ['BLOB_ACCOUNT_KEY']
    connect_str = f"DefaultEndpointsProtocol=https;AccountName={account_name};AccountKey={account_key};EndpointSuffix=core.windows.net"
    container_name = os.environ['BLOB_CONTAINER_NAME_PROMPTS']
    ...
    blob_list = container_client.list_blobs(include='metadata')
    # sas = generate_blob_sas(account_name, container_name, blob.name,account_key=account_key,  permission="r", expiry=datetime.utcnow() + timedelta(hours=3))
    sas = generate_container_sas(account_name, container_name,account_key=account_key,  permission="r", expiry=datetime.utcnow() + timedelta(hours=6))
    files = []
    converted_files = {}
    for blob in blob_list:
        if blob.name.endswith('.txt'):
            # read yaml file
            # with open(f"https://{account_name}.blob.core.windows.net/{container_name}/{blob.name}?{sas}") as file:
            #     prompt = file.read()
            lines = []
            # print(f"Opening: https://{account_name}.blob.core.windows.net/{container_name}/{blob.name}?{sas}")
            for line in urllib.request.urlopen(f"https://{account_name}.blob.core.windows.net/{container_name}/{blob.name}?{sas}"):
                lines.append(line.decode('utf-8')) #utf-8 or iso8859-1 or whatever the page encoding scheme is
                pass

            prompt = ''.join(lines)
            files.append({
                "filename" : blob.name,
                "prompt_name": blob.name.replace('.txt', ''),
                "fullpath": f"https://{account_name}.blob.core.windows.net/{container_name}/{blob.name}?{sas}",
                "converted_path": "",
                "prompt": prompt
                })
